...
//	
//	if (global_state.lights.rainbow_cycle.count < 256) { // cycle of all colors on wheel
//		for (uint16_t i = 0; i< strip.numPixels(); i++) {
...
void setup() {
	...
	//pinMode(button_music_dec, INPUT);
	pinMode(error_lib, INPUT);
	pinMode(error_sd, INPUT);
	pinMode(fan, OUTPUT);
	pinMode(light_inc, OUTPUT);
	pinMode(light_dec, OUTPUT);
	...
	digitalWrite(music_inc, global_state.music);
	digitalWrite(light_inc, global_state.lights.inc_state);
	digitalWrite(light_dec, global_state.lights.dec_state);
...
void loop() {
	...
	lcd.clear();
	lcd.print(String(height_print, 1));
	...
	//light control
	if (digitalRead(button_light_inc) == LOW && global_state.lights.inc_state == LOW) {
		global_state.lights.inc_state = HIGH;
		digitalWrite(light_inc, global_state.lights.inc_state);
		startVibe(vibe_light_inc);
	}

	if (global_state.lights.inc_state == HIGH && digitalRead(button_light_inc) != LOW) {
		global_state.lights.inc_state = LOW;
		digitalWrite(light_inc, global_state.lights.inc_state);
	}
	/*if (digitalRead(button_light_inc) == LOW && global_state.vibe.light_inc == 0) {
		/*if (global_state.lights.state.active) {
			global_state.lights.state.interrupt = 1;
		}*
		startVibe(vibe_light_inc);
		digitalWrite(light_inc, HIGH);
	...
	/*digitalWrite(light_dec, LOW);
	if (digitalRead(button_light_dec) == LOW && global_state.vibe.light_dec == 0) {
		/*if (global_state.lights.state.active) {
			global_state.lights.state.interrupt = 1;
			global_state.lights.state.interrupt_reset = 1;
		}*
		startVibe(vibe_light_dec);
		digitalWrite(light_dec, HIGH);
		/*global_state.lights.state.count--;
	...
	if (digitalRead(button_light_dec) == LOW && global_state.lights.dec_state == LOW) {
		global_state.lights.dec_state = HIGH;
		digitalWrite(light_dec, global_state.lights.dec_state);
		startVibe(vibe_light_dec);
	}

	if (global_state.lights.dec_state == HIGH && digitalRead(button_light_dec) != LOW) {
		global_state.lights.dec_state = LOW;
		digitalWrite(light_dec, global_state.lights.dec_state);
	}
	...
	//music control
	if (digitalRead(button_music_inc) == LOW && global_state.music == LOW) {
		global_state.music = HIGH;
		digitalWrite(music_inc, global_state.music);
		startVibe(vibe_music_inc);
	}

	if (global_state.music == HIGH && digitalRead(button_music_inc) != LOW) {
		global_state.music = LOW;
		digitalWrite(music_inc, global_state.music);
	}
