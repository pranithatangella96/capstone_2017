...
void loop() {
	...
	if ((digitalRead(up_button) == LOW) && (height < 46.8) && current_milleseconds - wrote_up > LEG_SERIAL_DELAY) {
			Serial1.write(command_up, 13);
			wrote_up = current_milleseconds;
	}
	if ((digitalRead(down_button) == LOW) && (height > 27.9) && current_milleseconds - wrote_down > LEG_SERIAL_DELAY) {
		Serial1.write(command_down, 13);
		wrote_down = current_milleseconds;
	}
